...
module.exports = {
	...
	registerUser: function (request, response) {
		...
		connection.query('SELECT * FROM user', function(error, rows, fields) {
			...
			if(currentUser) {
				if(CryptoJS.MD5(currentUser.salt + userPassword).toString() === currentUser.password.toString()) response.end("Logged in.")
				else response.end("User already exists or incorrect password.");
			}
			else
			{
				var generatedSalt = utils.generateRandomSalt(),
					post = {
						email: userEmail,
						salt: generatedSalt,
						password: CryptoJS.MD5(generatedSalt + userPassword).toString(),
						created: new Date()
					};
