...
function BattleScene(party, dbData, onBattleEndCallback, phaserGame) {
  ...
  // this._turn = ['_monsterUnits', '_partyUnits'][Math.round(Math.random())];
  this._turn = '_partyUnits';
  this._currentUnitOnTurn = this._battleGround[this._turn].getUnitsGroup().getFirstAlive();
...
BattleScene.prototype._randomizeMonsters = function() {
  ...
  possibleMonsterTypes.forEach(function(monsterType, index) {
    var shouldSpawn = Phaser.Utils.chanceRoll(monsterType.encounterchance);//index === 2;
    var countRange = _.object(['min', 'max'], monsterType.countrange.split('-'));
    var randomizedCount = this._randomizeMonsterCountForGivenRange(countRange);//1;
...
BattleScene.prototype._initBattleMenu = function() {
  ...
  this._onMenuSelect = function(menu, selectedOption) {
    ...
    var self = this;
    var currentUnitIndex = self._battleGround[self._turn].getUnitsGroup().cursorIndex;
    ...
    switch (selectedOption) {
      ...
      case 'ItemHP+++':
        ...
        this._battleGround[this._turn].activate(true, function(unit) {
          unit.act(selectedOption, null, function() {
            self._battleGround[self._turn].getUnitsGroup().cursorIndex = currentUnitIndex;
            self._endUnitTurn();
            menu.enabled = true;
          });
        });
...
BattleScene.prototype._endUnitTurn = function() {
  ...
  var currentUnitGroup = this._battleGround[this._turn].getUnitsGroup();
