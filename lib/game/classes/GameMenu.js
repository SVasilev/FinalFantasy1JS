  ...
  this.phaserGame.input.keyboard.onUpCallback = function(event) {
    ...
    case 'enter':
      ...
      var optionItem = _.find(this.options, function(item) {
        return item === currentChoiceText || (typeof item === 'object' && Object.keys(item)[0] === currentChoiceText);
      });
      if (typeof optionItem === 'string') {
        this.config.choiceCallback(this, (this.config._parent || '') + currentChoiceText);
      }
      else { // Open submenu
        this.enabled = false;
        ...
        new GameMenu(this.spriteKeys, optionItem[Object.keys(optionItem)[0]], subMenuConfig, this.phaserGame);
      }
      break;
    case escapeKey:
