...
GameMenu.prototype._attachKeyPressEvents = function(skipOne) {
  this.phaserGame.input.keyboard.onUpCallback = function(event) {
    if (!this.enabled || skipOne) {
      skipOne = false;
      return;
    }
    ...
    switch (pressedKey) {
      ...
      case 'enter':
        ...
        if (typeof choiceCallback === 'object') { // Open submenu
          ...
          subMenuConfig._parent = (this.config._parent || '') + currentChoiceText;
...
GameMenu.prototype.visible = function(action, skipOne) {
  ...
  action || skipOne && this._attachKeyPressEvents(skipOne);
};
