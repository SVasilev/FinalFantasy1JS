...
BattleUnits.prototype._init = function() {
  ...
  this._setCursorToUnitWithIndex(0);
...
BattleUnits.prototype._attachKeyPressEvents = function() {
  this.phaserGame.input.keyboard.onUpCallback = function(event) {
    ...
    if (pressedKey === 'enter') {
      this._onSelectCallback(this._unitsGroup.children[this._unitsGroup.cursorIndex]);
      return;
    }
    ...
    if (selectedUnit && !selectedUnit.alive) { // Check if the next unit in the row/column is alive.
      ...
      selectedUnit = this._unitsGroup.children[this._unitsGroup.cursorIndex + directionValue[pressedKey]];
    }

    selectedUnit && selectedUnit.alive && this._setCursorToUnitWithIndex(this._unitsGroup.cursorIndex + directionValue[pressedKey]);
  }.bind(this);
};

BattleUnits.prototype._setCursorToUnitWithIndex = function(index) {
...
BattleUnits.prototype._resetCursor = function() {
  ...
  firstAliveUnit && this._setCursorToUnitWithIndex(this._unitsGroup.cursorIndex);
};
...
BattleUnits.prototype.addUnit = function(unitSprite) {
  if (this._unitCount === BattleUnits['MAX_UNIT_COUNT']) {
    return;
  }
...
BattleUnits.prototype.nextAlive = function() {
  for (var i = this._unitsGroup.cursorIndex + 1; i < this._unitsGroup.length; i++) {
    if (this._unitsGroup.children[i].alive) {
      this._unitsGroup.cursorIndex = i;
      return this._unitsGroup.children[i];
    }
  }
};
