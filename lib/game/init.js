...
function create() {
  ...
  var worldMapPartySprites = {};
  ['walk', 'boat'].forEach(function(spriteKey) {
    worldMapPartySprites[spriteKey] = game.add.sprite(game.width / 2, game.height / 2 - 35, spriteKey);
    worldMapPartySprites[spriteKey].visible = false;
    // Attach config property to the current Phaser sprite.
    worldMapPartySprites[spriteKey].config = spritesData[spriteKey];
  });

  var walkSpriteScale = worldMapPartySprites['walk'].config.scale;
  worldMapPartySprites['walk'].visible = true;
  worldMapPartySprites['walk'].scale.setTo(walkSpriteScale.x, walkSpriteScale.y);
